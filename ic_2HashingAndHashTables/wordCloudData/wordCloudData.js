...
class WordCloudData {
  ...
  populateWordsToCounts(inputString) {
    // Iterates thru each char in input, splitting words and passing them to this.addWordToMap()
    let currentWordStartIndex = 0;
    let currentWordLength = 0;
    for (let i = 0; i < inputString.length; i++) {
      let char = inputString[i];
      // If we nreached the end of the string, we check if the last char is a letter and add the last word to our map
      if (i === inputString.length - 1) {
        if (this.isLetter(char)) {
          currentWordLength++;
        }
        if (currentWordLength > 0) {
          let word = inputString.slice(currentWordStartIndex, currentWordStartIndex + currentWordLength);
      ...
      // We want to make sure we split on ellipses so if we get two periods in a row, we add the current wordto our map and reset our currnet word
      } else if (char === '.') {
      ...
      // if char is a letter or apostrophe, add to current word
      } else if (this.isLetter(char) || char === '\'') {
        if (currentWordLength === 0) {
          currentWordStartIndex = i;
        }
        currentWordLength++;
      // If char is a hyphen, check if it's surrounded by letters.
      // If so, add to currWord
      } else if (char === '-') {
  ...
  addWordToMap(word) {
    ...
    // If a lowercase version if in the map, we know our input word must be uppercase, but we only include upsercase words if they're always uppercase.
      // Simply increment the lowercase version's count
    ...
    // Otherwise, word is not in map (lowercase/uppercase) => add to map
    } else {
