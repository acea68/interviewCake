...
function isFirstComeFirstServed(takeOutOrders, dineInOrders, servedOrders) {
  // check first order on servedOrders
  // if firstserved/.next === takeout/.next || dineIn/.next, advance respective indxs
    // otherwise, return false
  // return true if all iterations pass
  if (servedOrders.length !== takeOutOrders.length + dineInOrders.length) return false
  let tInd = 0, dInd = 0, sInd = 0;
  while (sInd < servedOrders.length) {
    if (servedOrders[sInd] === takeOutOrders[tInd] || dInd > dineInOrders.length) {
      tInd++;
    } else if (servedOrders[sInd] === dineInOrders[dInd] || tInd > takeOutOrders.length) {
      dInd++;
    } else {
      return false;
    }
