...
function isFirstComeFirstServed(takeOutOrders, dineInOrders, servedOrders) {
  ...
  let takeoutInd = dineinInd = servedInd = 0;
  while (takeoutInd < takeOutOrders.length || dineinInd < dineInOrders.length) {
    if (servedOrders[servedInd] === takeOutOrders[takeoutInd]) {
      takeoutInd++;
    } else if (servedOrders[servedInd] === dineInOrders[dineinInd]) {
      dineinInd++;
    } else {
      return false;
    }
